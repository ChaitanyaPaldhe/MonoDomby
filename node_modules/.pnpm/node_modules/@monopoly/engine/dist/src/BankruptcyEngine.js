...
;
const AssetTransferEngine_js_1 = require("./AssetTransferEngine.js");
const crypto_1 = require("crypto");
class BankruptcyEngine {
    static executeBankruptcyPlan(state, plan, config, actionId, clientTs) {
        let currentState = { ...state };
        const events = [];
        const playerId = plan.playerId;
        // 1. Emit BANKRUPTCY_STARTED
        events.push({
            id: (0, crypto_1.createHash)('sha256').update(`${actionId}:bankruptcy-start:${playerId}`).digest('hex'),
            type: shared_1.EventType.BANKRUPTCY_STARTED,
        ...
        const transferResult = AssetTransferEngine_js_1.AssetTransferEngine.transferAllAssets(currentState, playerId, plan.creditorId, actionId, clientTs, config);
        ...
        events.push({
            id: (0, crypto_1.createHash)('sha256').update(`${actionId}:player-eliminated:${playerId}`).digest('hex'),
            type: shared_1.EventType.PLAYER_ELIMINATED,
            roomId: currentState.roomId,
            gameId: currentState.id,
            ts: clientTs,
            payload: {
        ...
        events.push({
            id: (0, crypto_1.createHash)('sha256').update(`${actionId}:bankruptcy-resolved:${playerId}`).digest('hex'),
            type: shared_1.EventType.BANKRUPTCY_RESOLVED,
            roomId: currentState.roomId,
            gameId: currentState.id,
            ts: clientTs,
            payload: {
