...
import { TradeStatus } from '@monopoly/shared';
import type { GameState, PlayerId, TradeId, TradeOffer, TradeState } from '@monopoly/shared';
...
export class TradeEngine {
   ...
   */
  private executeTrade(state: GameState, trade: TradeState): GameState {
    // TODO: Implement
   ...
   */
  private requireTrade(state: GameState, tradeId: TradeId): TradeState {
    const trade = state.activeTrades[tradeId];
    if (!trade) {
      throw new EngineValidationError(
        `Trade '${tradeId}' not found.`,
        ErrorCode.E_TRADE_NOT_FOUND,
      );
    }
    return trade;
  }
   ...
   */
  private assertNotExpired(trade: TradeState, now: number): void {
    if (now >= trade.expiresAt) {
      throw new EngineValidationError(
        `Trade '${trade.id}' has expired.`,
        ErrorCode.E_TRADE_EXPIRED,
      );
    }
  }
}
