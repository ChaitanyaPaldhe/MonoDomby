...
export class ReplayRepository {
    ...
    async getMetadata(gameId) {
        const res = await this.client.query(`SELECT game_id, total_actions, duration_seconds, player_count, recorded_at 
       FROM replay_metadata WHERE game_id = $1`, [gameId]);
    ...
    async saveMetadata(metadata) {
        await this.client.query(`INSERT INTO replay_metadata (game_id, total_actions, duration_seconds, player_count)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (game_id) DO UPDATE SET 
         total_actions = EXCLUDED.total_actions,
         duration_seconds = EXCLUDED.duration_seconds,
         player_count = EXCLUDED.player_count,
         recorded_at = CURRENT_TIMESTAMP`, [metadata.gameId, metadata.totalActions, metadata.durationSeconds, metadata.playerCount]);
    }
    async getFullReplay(gameId) {
        ...
        const eventsRes = await this.client.query(`SELECT id, game_id, action_id, sequence_number, event_payload, created_at
       FROM events
       WHERE game_id = $1
       ORDER BY sequence_number ASC`, [gameId]);
