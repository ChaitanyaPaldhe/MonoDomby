...
// Auction subsystem.
// =============================================================================
import { AuctionStatus, ErrorCode, EventType, GamePhase } from '@monopoly/shared';
;
...
export class AuctionEngine {
    ...
    startAuction(state, tileId, mapConfig, now) {
        ...
        const event = this.createEvent(newState, EventType.PROPERTY_AUCTIONED_START, { tileId, startingBid, auction: auctionState }, now);
    ...
    placeBid(state, playerId, amount, mapConfig, now) {
        ...
        const events = [];
        events.push(this.createEvent(newState, EventType.AUCTION_BID_PLACED, {
        ...
        if (extended) {
            events.push(this.createEvent(newState, EventType.AUCTION_EXTENDED, {
                auctionId: auction.id,
                newEndsAt: endsAt,
                extensionCount,
            }, now));
    ...
    endAuction(state, mapConfig, now) {
        ...
        const events = [];
        let newState = { ...state };
        const completeEvent = this.createEvent(newState, EventType.AUCTION_COMPLETE, {
            auctionId: auction.id,
        ...
        if (auction.highestBidder !== null) {
            ...
            if (!winner)
                throw new EngineStateCorruptionError('Winner not found');
            // Deduct money
            newState = {
                ...newState,
                players: {
            ...
            const soldEvent = this.createEvent(newState, EventType.PROPERTY_AUCTIONED_SOLD, {
                tileId: auction.tileId,
                winnerId: winner.id,
                finalBid: auction.highestBid,
            }, now);
            ...
            // Unsold
            const unsoldEvent = this.createEvent(newState, EventType.PROPERTY_AUCTIONED_UNSOLD, {
                tileId: auction.tileId,
            }, now);
    ...
    requireActiveAuction(state) {
        if (!state.auction) {
            throw new EngineValidationError('No auction is currently active.', ErrorCode.E_INVALID_PHASE);
        }
        if (state.auction.status === AuctionStatus.COMPLETE) {
            throw new EngineValidationError('Auction has already ended.', ErrorCode.E_AUCTION_ENDED);
        }
