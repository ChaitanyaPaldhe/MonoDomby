...
// =============================================================================
import { TileType, CardDeckType, JailReason, TurnPhase, DecisionType, EventType, } from '@monopoly/shared';
;
...
export class TileResolver {
    ...
    resolve(state, tileIndex, config, action, actingPlayerId) {
        ...
        if (!tile) {
            throw new EngineStateCorruptionError(`TileResolver: No tile at index ${tileIndex} in MapConfig '${config.meta.id}'.`);
        }
    ...
    dispatchByType(state, tile, config, action, actingPlayerId) {
        switch (tile.type) {
            ...
            default: {
                ...
                throw new EngineStateCorruptionError(`TileResolver: Unknown tile type '${String(_exhaustive)}' on tile '${tile.id}'.`);
            }
        }
    }
    // =========================================================================
    ...
    resolveProperty(state, tile, config, action, actingPlayerId) {
        ...
        if (!tileState) {
            throw new EngineStateCorruptionError(`TileResolver: Missing BoardState for property tile '${tile.id}'.`);
        }
    ...
    resolveRailroad(state, tile, config, action, actingPlayerId) {
        ...
        const tileState = state.board.tiles[tileId];
        if (!tileState) {
            throw new EngineStateCorruptionError(`TileResolver: Missing BoardState for railroad tile '${tile.id}'.`);
        }
        if (tileState.ownerId === null) {
            return this.toPurchaseDecision(state, tileId);
        }
    ...
    resolveUtility(state, tile, config, action, actingPlayerId) {
        ...
        const tileState = state.board.tiles[tileId];
        if (!tileState) {
            throw new EngineStateCorruptionError(`TileResolver: Missing BoardState for utility tile '${tile.id}'.`);
        }
        if (tileState.ownerId === null) {
            return this.toPurchaseDecision(state, tileId);
        }
    ...
    resolveCommunityChest(state, config, action, actingPlayerId) {
        return this.drawCardAndTransition(state, CardDeckType.COMMUNITY_CHEST, config, action, actingPlayerId);
    }
    // =========================================================================
    //  TileType.TAX
    // =========================================================================
    ...
    resolveGoToJail(state, config, action, actingPlayerId) {
        ...
        if (!player) {
            throw new EngineStateCorruptionError(`TileResolver.resolveGoToJail: player '${actingPlayerId}' not in state.`);
        }
        const newJailState = {
        ...
        return {
            newState,
            events: [
                TileResolver.buildPlayerJailedEvent(state, action, actingPlayerId, JailReason.GO_TO_JAIL_TILE),
            ],
        };
    }
    // =========================================================================
    ...
    drawCardAndTransition(state, deckType, config, action, actingPlayerId) {
        ...
        if (pile.length === 0) {
            if (discard.length === 0) {
                throw new EngineStateCorruptionError(`TileResolver: ${deckType} deck has zero cards in both pile and discard.`);
            }
