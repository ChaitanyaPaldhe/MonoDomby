import type { GameState, PlayerId, TradeId, TradeOffer } from '@monopoly/shared';
 ...
 *     REJECTED / CANCELLED
 */
export declare class TradeEngine {
    /**
     ...
     */
    proposeTrade(state: GameState, initiatorId: PlayerId, targetId: PlayerId, offer: TradeOffer, request: TradeOffer, mapConfig: MapConfig, now: number): EngineResult;
    /**
     ...
     * @param now - Server timestamp (unix ms).
     */
    counterTrade(state: GameState, playerId: PlayerId, tradeId: TradeId, newOffer: TradeOffer, newRequest: TradeOffer, mapConfig: MapConfig, now: number): EngineResult;
    /**
     ...
     * @param now - Server timestamp (unix ms).
     */
    acceptTrade(state: GameState, playerId: PlayerId, tradeId: TradeId, mapConfig: MapConfig, now: number): EngineResult;
    /**
     ...
     */
    rejectTrade(state: GameState, playerId: PlayerId, tradeId: TradeId, now: number): EngineResult;
    /**
     ...
     * @param now - Server timestamp.
     */
    cancelTrade(state: GameState, playerId: PlayerId, tradeId: TradeId, now: number): EngineResult;
    /**
     ...
     * @param now - Server timestamp (unix ms).
     */
    expireOldTrades(state: GameState, now: number): EngineResult;
    /**
