...
class WinDetector {
     ...
     */
    check(state, mapConfig) {
        const { winCondition } = mapConfig.rules;
        switch (winCondition) {
            case shared_1.WinCondition.LAST_STANDING:
                return this.checkLastStanding(state);
            case shared_1.WinCondition.NET_WORTH_TARGET:
                return this.checkNetWorthTarget(state, mapConfig);
            case shared_1.WinCondition.TURN_LIMIT:
                return this.checkTurnLimit(state, mapConfig);
            default: {
                // TypeScript exhaustive check
                const _exhaustive = winCondition;
    ...
    checkLastStanding(state) {
        const activePlayers = state.playerOrder.filter(id => state.players[id] && !state.players[id].isBankrupt);
     ...
     */
    checkNetWorthTarget(state, mapConfig) {
     ...
     */
    checkTurnLimit(state, mapConfig) {
...
exports.WinDetector = WinDetector;
//# sourceMappingURL=WinDetector.js.map