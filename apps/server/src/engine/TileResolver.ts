 ...
 */
export type CustomTileHandlerFn = (
  state: GameState,
  tile: Tile,
  config: MapConfig,
  action: ClientAction,
...
export class TileResolver {
   ...
   */
  private readonly customHandlers: ReadonlyMap<string, CustomTileHandlerFn>;
  ...
  resolve(
    ...
    const tile: Tile | undefined =
      fastLookup?.index === tileIndex
        ? fastLookup
        : config.board.tiles.find(t => t.index === tileIndex);

    if (!tile) {
      throw new EngineStateCorruptionError(
        `TileResolver: No tile at index ${tileIndex} in MapConfig '${config.meta.id}'.`,
      );
    }
  ...
  private dispatchByType(
    ...
    switch (tile.type) {
      ...
      default: {
        ...
        const _exhaustive: never = tile.type;
        throw new EngineStateCorruptionError(
          `TileResolver: Unknown tile type '${String(_exhaustive)}' on tile '${tile.id}'.`,
        );
      }
    }
  }
   ...
   * → POST_ROLL
   */
  private resolveGo(state: GameState): EngineResult {
   ...
   * → POST_ROLL
   */
  private resolveJailVisit(state: GameState): EngineResult {
    return { newState: this.toPostRoll(state), events: [] };
  }
   ...
   */
  private resolveFreeParking(state: GameState, _config: MapConfig): EngineResult {
    return { newState: this.toPostRoll(state), events: [] };
  }
  ...
  private resolveProperty(
    ...
    if (!tileState) {
      throw new EngineStateCorruptionError(
        `TileResolver: Missing BoardState for property tile '${tile.id}'.`,
      );
    }
  ...
  private resolveRailroad(
    ...
    if (!tileState) {
      throw new EngineStateCorruptionError(
        `TileResolver: Missing BoardState for railroad tile '${tile.id}'.`,
      );
    }
  ...
  private resolveUtility(
    ...
    if (!tileState) {
      throw new EngineStateCorruptionError(
        `TileResolver: Missing BoardState for utility tile '${tile.id}'.`,
      );
    }
  ...
  private resolveChance(
    ...
    return this.drawCardAndTransition(state, CardDeckType.CHANCE, config, action, actingPlayerId);
  }
  ...
  private resolveCommunityChest(
    ...
    return this.drawCardAndTransition(
      state, CardDeckType.COMMUNITY_CHEST, config, action, actingPlayerId,
    );
  }
   ...
   */
  private resolveTax(state: GameState, _tile: Tile): EngineResult {
    return { newState: this.toPostRoll(state), events: [] };
  }
  ...
  private resolveGoToJail(
    ...
    if (!player) {
      throw new EngineStateCorruptionError(
        `TileResolver.resolveGoToJail: player '${actingPlayerId}' not in state.`,
      );
    }

    const newJailState: JailState = {
      reason: JailReason.GO_TO_JAIL_TILE,
      turnsServed: 0,
    ...
    return {
      newState,
      events: [
        TileResolver.buildPlayerJailedEvent(
          state, action, actingPlayerId, JailReason.GO_TO_JAIL_TILE,
        ),
      ],
    };
  }
  ...
  private drawCardAndTransition(
    ...
    const isChance = deckType === CardDeckType.CHANCE;
    ...
    if (pile.length === 0) {
      if (discard.length === 0) {
        throw new EngineStateCorruptionError(
          `TileResolver: ${deckType} deck has zero cards in both pile and discard.`,
        );
      }
      const [reshuffled, nextRng] = DiceEngine.shuffle(discard, rngState);
    ...
    const newDecks: GameState['cardDecks'] = isChance
    ...
    return {
      newState,
      events: [
        TileResolver.buildCardDrawnEvent(
          state, action, actingPlayerId, cardId, cardText, deckType,
        ),
      ],
    };
  }
  ...
  private toPurchaseDecision(state: GameState, tileId: TileId): EngineResult {
    const pendingDecision: PendingDecision = {
  ...
  private toPostRoll(state: GameState): GameState {
    return {
      ...state,
      turn: {
        ...state.turn,
