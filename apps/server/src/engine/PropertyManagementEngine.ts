...
export class PropertyManagementEngine {
  /**
  ...
  public static calculateNetWorth(
    playerMoney: number,
    playerProperties: readonly string[],
    boardTiles: Record<string, import('@monopoly/shared').TileState>,
    mapConfig: MapConfig
  ): number {
  ...
  public static applyTransaction(
    ...
    if (p) {
      players[playerId] = { ...p, money: p.money + plan.playerMoneyChange } as import('@monopoly/shared').PlayerState;
    }

    // 3. Apply Tile changes
    for (const [tileId, tileState] of Object.entries(plan.tileChanges)) {
      boardTiles[tileId as import('@monopoly/shared').TileId] = tileState;
    }
    ...
    for (const pId of Object.keys(players)) {
      ...
      players[pId as PlayerId] = { ...p, netWorth } as import('@monopoly/shared').PlayerState;
    }
