import { GameState, MapConfig, PlayerId, TileId, EventType, ErrorCode } from '@monopoly/shared';
import type { MortgagePlan } from './types.js';
import { EngineValidationError } from './errors.js';

export class MortgagePlanner {
  public static planMortgageProperty(
    state: GameState,
    config: MapConfig,
    tileId: TileId,
    playerId: PlayerId,
    ...
    if (!tileState) throw new EngineValidationError('Tile does not exist', ErrorCode.E_INVALID_ACTION);
    if (tileState.ownerId !== playerId) throw new EngineValidationError('Player does not own this property', ErrorCode.E_PROPERTY_NOT_OWNED);
    if (tileState.isMortgaged) throw new EngineValidationError('Property is already mortgaged', ErrorCode.E_INVALID_ACTION);
    ...
    if (!tileDef) throw new EngineValidationError('Tile definition not found', ErrorCode.E_INVALID_ACTION);
    ...
    if (tileDef.propertyData) {
      ...
      for (const t of groupTiles) {
        const ts = state.board.tiles[t.id as TileId];
  ...
  public static planUnmortgageProperty(
    state: GameState,
    config: MapConfig,
    tileId: TileId,
    playerId: PlayerId,
    ...
    if (!tileState) throw new EngineValidationError('Tile does not exist', ErrorCode.E_INVALID_ACTION);
    if (tileState.ownerId !== playerId) throw new EngineValidationError('Player does not own this property', ErrorCode.E_PROPERTY_NOT_OWNED);
    if (!tileState.isMortgaged) throw new EngineValidationError('Property is not mortgaged', ErrorCode.E_INVALID_ACTION);
  ...
  public static validatePlan(
    ...
    if (!ts) throw new EngineValidationError('Tile does not exist', ErrorCode.E_INVALID_ACTION);
    if (ts.ownerId !== playerId) throw new EngineValidationError('Ownership mismatch', ErrorCode.E_PROPERTY_NOT_OWNED);
    
    if (plan.isMortgaging) {
      if (ts.isMortgaged) throw new EngineValidationError('Already mortgaged', ErrorCode.E_INVALID_ACTION);
      if (ts.houses > 0 || ts.hasHotel) throw new EngineValidationError('Buildings exist', ErrorCode.E_INVALID_ACTION);
    } else {
      if (!ts.isMortgaged) throw new EngineValidationError('Not mortgaged', ErrorCode.E_INVALID_ACTION);
      const player = state.players[playerId]!;
